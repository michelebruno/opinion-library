...
function HoverImage({children, component: Component, image, imageUrl, ...props}) {
    ...
    let imgClassName = "  absolute top-2 right-6 translate-x-full z-[-1] opacity-50 transition-all"


    return <span className="relative group">
        <span className={"underline"} {...props} onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
            {children}
        </span>
        {showImage && image && <Image image={image} className={imgClassName} style={{minWidth: '20vw'}}/>}
        {showImage && imageUrl && <img src={imageUrl} className={imgClassName} style={{minWidth: '20vw'}}/>}
...
export default function About({data: {images: {nodes: images}, meme, team: {nodes: team}}}) {
    ...
    return <Layout wrapperClassName={"bg-white text-black about-page scroll-p-16"} container footer light>
            ...
            <article className={"col-span-2 relative z-10"}>
                ...
                <section id="data">
                    ...
                    <p>
                        ...
                        prompts users to explain why they signed right after they do so. This means there are no
                        comments
                    ...
                    <div>
                        <Image image={images.find(i => i.relativePath === 'about/tags.png')}
                               className={"mx-auto w-7/12"}/>
                    </div>
                    <p>We then made a list of all the petitions presenting at least one of the tags above. Since there
                        was
                        no easy way to do this from the website, we wrote a script in python to do this for us.
                        The script sends requests to change.org's server-side API, asking for all the petitions tagged
                        with
                    ...
                    <p>
                        ...
                        petitions, we could use their ID to ask the API for their comments. Once again, the server
                        answered
                        back with a *.json file containing all the comments for each petition.
                    </p>

                    <div
                        className="text-base w-full overflow-hidden border-2 border-black rounded-3xl w-7/12 mx-auto">
                        <h3 className="bg-black text-white px-8 py-4 sticky top-0 left-0 select-none">
                    ...
                    <p>
                        Once all the comments were gathered, we used Google Natural Language API to extract entities
                        from
                        the dataset — these are words and n-grams. This was a quick and easy way to get rid of
                        punctuation,
                    ...
                    <p>
                        ...
                        both groups or if one used it more than the other. We used the following proportion to transform
                        the
                ...
                <section id="interactions">
                    <h2>Interactions</h2>
                    <p>
                        The archive presents two kinds of information: how often a word is used by a certain group and
                        which
                        are the comments mentioning that word. Presenting both information to the same panel would
                        result in
                    ...
                    <p>
                        For this reason we decided to present the information in two different screens: related words
                        and
                        opinions. Such a division also allowed us to give sequentiality to the navigation, forcing the
                        user
                        to see how a word is used before reading the comments.
                        This offers preliminary information to the user while they’re deciding which comments to read
                        while
