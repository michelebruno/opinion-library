...
export default function useMatter(containerRef) {
  ...
  useEffect(() => {
    ...
    const scrollDownBox = Bodies.rectangle(sizeX / 2, sizeY, sizeX / 4, sizeY / 7, {
      isStatic: true,
      render: {
        visible: false,
      },
    ...
    const wall_top = Bodies.rectangle(window.innerWidth / 2, -6, window.innerWidth, 10, {
      isStatic: true,
    });
    const wall_left = Bodies.rectangle(-6, window.innerHeight / 2, 10, window.innerHeight, {
      isStatic: true,
    });
