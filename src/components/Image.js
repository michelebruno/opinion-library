...
const Image = forwardRef((props, ref) => {
  ...
  const {alt, className, pictureClassName, loading, preload, id, onClick, style} = props;

  const {height, width, images} = image;

  return (
    <picture className={classNames('mb-image', pictureClassName)} id={id}>
      {preload && (
        <Helmet
          link={[
      ...
      {images.sources.map(({sizes, srcSet, type}) => (
        <source type={type} srcSet={srcSet} sizes={sizes} key={type} />
      ))}
      <img
        ref={ref}
        alt={alt}
